...
function statusPillClass(status: string) {
  ...
  if (s === "completed") return "bg-sky-500/15 text-sky-300 border-sky-500/40";
...
> = (toolbar) => {
  ...
  const goPrev = () =>
    toolbar.onDateChange(new Date(current.getFullYear(), currentMonth - 1, 1));
  const goNext = () =>
    toolbar.onDateChange(new Date(current.getFullYear(), currentMonth + 1, 1));
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 p-3 sm:p-4 bg-neutral-900/70 backdrop-blur-md rounded-xl border border-neutral-800 shadow-md">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={goToday}
          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm rounded-md transition"
        >
          Today
        </button>
...
const CustomDateHeader: React.FC<CustomDateHeaderProps> = ({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {daySummary && (
        <span className="inline-flex items-center gap-1 rounded-full bg-neutral-900/90 border border-neutral-700 px-1.5 py-0.5 text-[9px] sm:text-[10px] text-neutral-100">
          <span className="h-1 w-1 rounded-full bg-blue-400" />
          {daySummary.total} appt
        </span>
      )}
    </div>
  );
...
export default function CalendarWidget() {
  ...
  const legend = [
    ...
    { label: "Cancelled / Rejected", color: "#ef4444" },
  ];

  // ---------- UI ----------
  return (
    <div className="w-full">
      <div className="rounded-2xl border border-neutral-800 bg-gradient-to-b from-[#111113] to-[#0b0b0c] p-4 sm:p-6 shadow-[0_0_25px_rgba(0,0,0,0.4)] backdrop-blur-sm">
        {/* Header */}
        ...
        <div className="overflow-hidden rounded-xl border border-neutral-800">
          ...
          >
            <RBC<Appointment>
              ...
              className="text-[11px] sm:text-sm [&_.rbc-month-view]:bg-neutral-900 [&_.rbc-today]:bg-blue-500/10 [&_.rbc-off-range-bg]:bg-neutral-800/40 [&_.rbc-date-cell]:text-neutral-300"
            />
          </div>
        </div>
        ...
        {selectedEvent && (
          ...
          >
            ...
            <div className="mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <Button className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-90 text-white rounded-md transition">
                Edit Appointment
              </Button>
            </div>
          </div>
