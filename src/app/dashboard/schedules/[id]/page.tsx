...
export default function EditSchedulePage() {
  ...
  useEffect(() => {
    ...
    const loadSchedule = async () => {
      try {
        ...
        const rows: WeekRow[] = WEEK_DAYS.map((d, idx) => {
          const existing = weekByDay[d.key];
          if (existing) {
            return {
              key: `${now}-${d.key}-${idx}`,
              day: existing.day,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateScheduleApi(id, payload as any);
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.8fr)_minmax(0,1.1fr)]">
        ...
        <div className="space-y-5">
          <SectionCard
            ...
            subtitle="Quick overview of what you’re editing."
          >
